...
const InterviewCard = ({ interview, onClick }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`text-left rounded-2xl p-5 ${style.bg} hover:brightness-95 transition cursor-pointer`}
    >
      ...
      {isPending ? (
        <p className="flex items-center gap-1.5 text-sm text-charcoal/60 mt-1">
          <Mail size={13} /> Invited: {interview.candidateEmail}
        </p>
      ) : (
        <p className="text-sm text-charcoal/60 mt-1">with {otherParty?.name || "—"}</p>
      )}
      ...
      {isPending && (
        <span className="inline-block mt-3 text-xs font-medium text-coral bg-coral/10 rounded-full px-2.5 py-1">
          Pending signup
        </span>
      )}
    </button>
