...
const CodeList = () => {
  ...
  useEffect(() => {
    api
      .get("/interviews")
      .then((data) => setInterviews(data.filter((i) => i.candidate)))
      .finally(() => setLoading(false));
  }, []);

  return (
    <div>
      <h1 className="font-display font-extrabold text-3xl">Code Editor</h1>
      <p className="text-charcoal/50 mt-2">Open a shared coding session for an interview.</p>

      {loading ? (
        <p className="text-charcoal/50 mt-8">Loading…</p>
      ) : interviews.length === 0 ? (
        <p className="text-charcoal/50 mt-8">No interviews available yet.</p>
      ) : (
            ...
            return (
              ...
              >
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${style.badge}`}>
                  <Code2 size={18} className="text-charcoal" />
                </div>
                <div>
